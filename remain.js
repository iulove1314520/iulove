...
function operator(proxies, targetPlatform) {
  return proxies
    .map(proxy => {
      ...
      proxy.name = proxy.name
        // 4.1 运营商重命名为 HOME
        .replace(/\b(Comcast|Maxis|Sejong|NTT|CTM|HGC|Rakuten|HKT|HKBN|HiNet|Seednet|M1|CAT|Exetel|Biglobe|KDDI|SoNet|SoftBank|TM|KT|SK|LG)\b/gi, 'HOME')
      ...
      proxy.name = proxy.name
        ...
        .replace(/\b(Czech Republic|Prague)\b/g, '捷克')
