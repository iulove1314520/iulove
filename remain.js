...
function operator(proxies, targetPlatform) {
  return proxies
    .map(proxy => {
      ...
      proxy.name = proxy.name
        // 3.1 删除供应商标记
        .replace(/\b(UDPN|HUAWEI|DMIT|Eons|Gcore|Jinx|Nearoute|Cloudflare|Misaka|Sakura|DigitalOcean|AWS|SG\.GS|Akile|Akari|PQS|Apol|Bangmod|Oracle|Linode|Gbnetwork|Webconex|Eastern|Aliyun|Google Cloud|Microsoft Azure|Vultr|OVH|Hetzner|Tencent Cloud|IDCloudhost|UpCloud|Scaleway|Rackspace|HostGator|GoDaddy|DreamHost|Fastly|Bluehost|InMotion|Kinsta|Namecheap|Hostinger)\b/gi, '')
        // 3.2 删除通用标记
      ...
      proxy.name = proxy.name
        ...
        .replace(/\b(TR|Turkey|Istanbul|Ankara)\b/g, '土耳其')         // 添加伊斯坦布尔和安卡拉
